...
bar(categorical(["商品1", "商品2"]), [100,80], ...
    FaceColor='flat', CData=lines(2)); % lines(2) は２つデフォルト色を返すコマンド（2x3 double)
ylim([0, 130]);
ylabel('日別販売数（平均）');
...
<center><img src="chapter2_3_media/figure_0.png" width="728" alt="figure_0.png"></center>
...
% バイオリンプロット
nexttile
% "violinplot"はMATLAB製品（R2023b 時点）として関数がないため除外します
% 以下の関数でも実現可能
...
<center><img src="chapter2_3_media/figure_1.png" width="728" alt="figure_1.png"></center>
...
data = table([type1; type2], [category1;category2], VariableNames=["type","value"]);
data4 = data(data.type=="商品4",:);  % 商品4だけを選択
data4.type = categorical(data4.type); % ここでカテゴリ型に変換
